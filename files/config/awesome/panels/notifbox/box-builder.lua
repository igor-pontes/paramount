...
local builder = require("panels.notifbox.box-ui-elements")
local notifbox_core = require("panels.notifbox")
...
local parse_to_seconds = function(time)
	local hourInSec = tonumber(string.sub(time, 1, 2)) * 3600 local minInSec = tonumber(string.sub(time, 4, 5)) * 60
...
notifbox_box = function(notif, icon, title, message, app, bgcolor)
	...
	local notifbox_template = wibox.widget({
		...
		shape = function(cr, width, height)
			gears.shape.partially_rounded_rect(cr, width, height, true, true, true, true, dpi(2))
		end,
